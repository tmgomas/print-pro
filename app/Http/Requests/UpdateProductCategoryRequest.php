...
class UpdateProductCategoryRequest extends FormRequest
{
    ...
    {
        return $this->user() && $this->user()->can('edit product categories');
    }
    ...
    {
        $categoryId = $this->route('id') ?? $this->route('product_category') ?? $this->route('category');
        ...
        return [
            ...
            'parent_id' => [
                ...
                function ($attribute, $value, $fail) use ($categoryId) {
                    if ($value) {
                        ...
                        if ($this->hasCircularReference($categoryId, $value)) {
            ...
            'status' => [
                ...
                'in:active,inactive',
                function ($attribute, $value, $fail) use ($categoryId) {
                    // If deactivating, check if it has active child categories
                    if ($value === 'inactive') {
                        $hasActiveChildren = \App\Models\ProductCategory::where('parent_id', $categoryId)
                            ->where('status', 'active')
                            ->exists();
                        
                        if ($hasActiveChildren) {
                            $fail('Cannot deactivate category with active subcategories. Please deactivate subcategories first.');
                        }

                        // Check if it has active products
                        $hasActiveProducts = \App\Models\Product::where('category_id', $categoryId)
                            ->where('status', 'active')
                            ->exists();
                        
                        if ($hasActiveProducts) {
                            $fail('Cannot deactivate category with active products. Please move or deactivate products first.');
                        }
                    }
                },
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'description.string' => 'විස්තරය වලංගු text එකක් විය යුතුයි. / Description must be valid text.',
            ...
            'parent_id.not_in' => 'කැටගරියක් තමන්ම parent කරගත නොහැක. / A category cannot be parent of itself.',
    ...
    protected function prepareForValidation(): void
    {
        $this->merge([
            ...
            'code' => $this->code ? strtoupper(preg_replace('/[^A-Za-z0-9_-]/', '', trim($this->code))) : null,
     ...
     */
    private function hasCircularReference(int $categoryId, int $parentId): bool
    {
        $visited = [];
        $current = $parentId;
        ...
        while ($current && !in_array($current, $visited)) {
            ...
            $category = \App\Models\ProductCategory::find($current);
